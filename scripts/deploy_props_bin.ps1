...
function Copy-ToServer {
    ...
    ssh "$User@$Server" "cmd /c $mkdirCmd"
    ...
    $remoteSpec = "${User}@${Server}:`"$RemoteRoot`""
    scp -r -p $localSpec (Split-Path $RemoteRoot -Parent | ForEach-Object { "${User}@${Server}:`"$_`"" })
    if ($LASTEXITCODE -ne 0) {
...
function Assert-RemoteSizeMatches([long]$expectedBytes) {
    ...
    # PowerShell over ssh — sum sizes of all props.bin files in the remote tree.
    $remoteCmd = "powershell -NoProfile -Command `"(Get-ChildItem -Path '$RemoteRoot' -Recurse -Filter 'props.bin' | Measure-Object -Sum Length).Sum`""
    $remoteOut = ssh "$User@$Server" $remoteCmd
    if ($LASTEXITCODE -ne 0) {
...
function Restart-ServerTask {
    ...
    ssh "$User@$Server" "schtasks /End /TN `"$TaskName`""
    # Don't hard-fail if End returned non-zero — task may not have been running.
    ssh "$User@$Server" "schtasks /Run /TN `"$TaskName`""
    if ($LASTEXITCODE -ne 0) {
