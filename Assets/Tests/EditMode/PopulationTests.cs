...
namespace ForeverEngine.Tests
{
    public class PopulationTests
    {
        [Test] public void Population_HasPlayerSpawn()
        {
            ...
            var req = new GenerationRequest { Width = 64, Height = 64, Seed = 42, PartyLevel = 3, PartySize = 4 };
        ...
        [Test] public void Population_EncountersScaleWithLevel()
        {
            ...
            var low = PopulationGenerator.Populate(graph, new GenerationRequest { Width=128, Height=128, Seed=42, PartyLevel=1, PartySize=4 }, MapProfile.Get("dungeon"), new bool[128*128], 128);
            var high = PopulationGenerator.Populate(graph, new GenerationRequest { Width=128, Height=128, Seed=42, PartyLevel=10, PartySize=4 }, MapProfile.Get("dungeon"), new bool[128*128], 128);
        ...
        [Test] public void Population_DressingAlwaysPresent()
        {
            ...
            RoomPlacer.Place(graph, 64, 64, 42);
            foreach (var n in graph.Nodes) n.Purpose = "guard_room";
            var result = PopulationGenerator.Populate(graph, new GenerationRequest { Width=64, Height=64, Seed=42 }, MapProfile.Get("dungeon"), new bool[64*64], 64);
